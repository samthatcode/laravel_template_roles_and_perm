...
const store = () => {
    return form.post(route("super_admin.user.store"), {
        onSuccess: () => close(),
        onError: () => show(),
    });
};
...
const update = () => {
    return form.patch(route("super_admin.user.update", form.id), {
        onSuccess: () => close(),
        onError: () => show(),
    });
};
...
const destroy = async (user) => {
    ...
    if (response.isConfirmed) {
        ...
        return Inertia.delete(route("super_admin.user.destroy", user.id));
    }
};
...
const detachPermission = async (user, permission) => {
    ...
    Inertia.on("finish", () => close());
    Inertia.patch(
        route("super_admin.user.permission.detach", {
            user: user.id,
...
const detachRole = async (user, role) => {
    ...
    Inertia.on("finish", () => close());
    Inertia.patch(
        route("super_admin.user.role.detach", { user: user.id, role: role.id })
    );
};
...
<template>
    <DashboardLayout :title="__('user')">
        <Card class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100">
            <template #header>
                <div
                    class="flex items-center space-x-2 p-2 bg-gray-200 dark:bg-gray-800"
                >
                    <ButtonGreen
            ...
            <template #body>
                <div class="flex flex-col space-y-2">
                    <Builder
                        v-if="render"
                        :url="route('super_admin.user.paginate')"
                        ref="table"
                    >
                        <template #thead="table">
                            <tr
                                class="bg-gray-200 dark:bg-gray-800 border-gray-300 dark:border-gray-900"
                            >
                                <Th
                                    :table="table"
                                    :sort="false"
                            ...
                            >
                                <template v-if="empty">
                                    <tr>
                                        ...
                                        >
                                            <p
                                                class="lowercase first-letter:capitalize font-semibold"
                                            >
                                                {{
                                                    __(
                                ...
                                <template v-else>
                                    ...
                                    >
                                        <td
                                            class="px-2 py-1 border dark:border-gray-800 text-center"
                                        >
                                            {{ i + 1 }}
                                        </td>

                                        <td
                                            class="px-2 py-1 border dark:border-gray-800 uppercase"
                                        >
                                            {{ user.name }}
                                        </td>
                                        ...
                                        <td
                                            class="px-2 py-1 border dark:border-gray-800"
                                        >
                                            <div class="flex-wrap">
                                                <div
                                                    v-for="(
        ...
        <Modal :show="open">
            ...
            >
                <Card class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100">
                    <template #header>
                        <div
                            class="flex items-center justify-end bg-gray-200 dark:bg-gray-800 p-2"
                        >
                        ...
                        <div class="flex flex-col space-y-4 p-4">
                            <div class="flex flex-col space-y-2">
                                <div class="flex items-center space-x-2">
                                    <label
                                        for="name"
                                        class="w-1/3 lowercase first-letter:capitalize"
                                    >
                                        {{ __("name") }}
                                    </label>
                    ...
                    <template #footer>
                        <div
                            class="flex items-center justify-end space-x-2 bg-gray-200 dark:bg-gray-800 px-2 py-1"
                        >
