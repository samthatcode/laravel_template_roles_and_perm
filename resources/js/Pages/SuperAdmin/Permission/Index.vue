...
const fetch = async () => {
    try {
        const response = await axios.get(route("super_admin.permission"));
...
const store = () =>
    form.post(route("super_admin.permission.store"), {
...
const update = () =>
    form.patch(route("super_admin.permission.update", form.id), {
        onSuccess: () => close() || fetch(),
        onError: show,
    });
...
const destroy = async (permission) => {
    ...
    if (response.isConfirmed) {
        return Inertia.delete(
            route("super_admin.permission.destroy", permission.id),
            {
                onSuccess: fetch,
            }
        );
...
<template>
    <DashboardLayout :title="__('permission')">
        <Card class="bg-gray-50 dark:bg-slate-700 shadow-md">
            <template #header>
                <div
                    class="flex items-center space-x-2 bg-gray-200 dark:bg-gray-800 p-2"
                >
                    <ButtonGreen
            ...
            <template #body>
                <div
                    class="flex flex-col space-y-2 p-4 h-screen max-h-96 overflow-auto"
                >
                    <div
                        class="flex items-center justify-end space-x-2 text-sm dark:text-gray-100 px-4"
                    >
                        <input
                            v-model="search"
                    ...
                    <div
                        class="flex-wrap px-4 pb-2 dark:bg-gray-700 dark:text-gray-100 rounded-b-md"
                    >
                        <TransitionGroup
                        ...
                        >
                            <div
                                v-for="(permission, i) in permissions.filter(
                                    (p) =>
                                        p.name
                                            ?.toLowerCase()
                                            .includes(
        ...
        <Modal :show="open">
            ...
            >
                <Card
                    class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100 border dark:border-gray-700"
                >
                    <template #header>
                        <div
                            class="flex items-center space-x-2 justify-end bg-gray-200 dark:bg-gray-800 dark:text-gray-50 p-2"
                        >
                        ...
                        <div class="flex flex-col space-y-4 p-4">
                            <div class="flex flex-col space-y-2">
                                <div class="flex items-center space-x-2">
                                    <label
                                        for="name"
                                        class="lowercase first-letter:capitalize flex-none w-1/4"
                                    >
                                        {{ __("name") }}
                                    </label>
                    ...
                    <template #footer>
                        <div
                            class="flex items-center space-x-2 justify-end bg-gray-200 dark:bg-gray-800 text-white px-2 py-1"
                        >
