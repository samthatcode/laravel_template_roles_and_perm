...
const fetch = async () => {
    try {
        const response = await axios.get(route("super_admin.menu"));
...
const store = () => {
    return form.post(route("super_admin.menu.store"), {
        onSuccess: () => {
            close();
            form.reset();
...
const update = () => {
    return form.patch(route("super_admin.menu.update", form.id), {
        onSuccess: () => {
            close();
            form.reset();
            Inertia.get(route(route().current()));
...
const destroy = async (menu) => {
    ...
    response.isConfirmed &&
        Inertia.delete(route("super_admin.menu.destroy", menu.id), {
...
const save = () => {
    ...
    timeout.value = setTimeout(() => {
        return useForm({ menus: menus.value }).patch(
            route("super_admin.menu.save"),
            {
                onFinish: () => {
...
<template>
    <DashboardLayout :title="__('menu')">
        ...
        <Modal :show="open">
            ...
            >
                <Card class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100">
                    <template #header>
                        <div
                            class="flex items-center space-x-2 p-2 justify-end bg-gray-200 dark:bg-gray-800"
                        >
                        ...
                        <div class="flex flex-col space-y-2 p-4">
                            <div class="flex flex-col space-y-1">
                                <div class="flex items-center space-x-2">
                                    <label
                                        for="name"
                                        class="lowercase first-letter:capitalize w-1/3"
                                    >
                                        {{ __("name") }}
                                    </label>
                            ...
                            <div class="flex items-center space-x-2">
                                <label
                                    class="flex-none lowercase first-letter:capitalize w-1/4"
                                >
                                    {{ __("icon") }}
                                </label>
                    ...
                    <template #footer>
                        <div
                            class="flex items-center justify-end space-x-2 bg-gray-200 dark:bg-gray-800 py-1 px-2"
                        >
        ...
        <Modal :show="icon">
            <Card
                class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100 w-full max-w-xl sm:max-w-5xl h-fit"
            >
                <template #header>
                    <div
                        class="flex items-center space-x-2 p-2 justify-end bg-gray-200 dark:bg-gray-800"
                ...
                <template #body>
                    <div class="flex-wrap p-4 max-h-96 overflow-auto">
                        <Icon
                            v-for="(icx, i) in icons.filter((icx) =>
                                icx.includes(search.trim().toLocaleLowerCase())
                            )"
                            :key="i"
                            @click.prevent="
                                form.icon = icx;
