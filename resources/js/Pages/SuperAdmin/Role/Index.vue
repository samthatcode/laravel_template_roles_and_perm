...
const store = () => {
    return form.post(route("super_admin.role.store"), {
        onSuccess: () => close(),
        onError: () => show(),
    });
};
...
const update = () => {
    return form.patch(route("super_admin.role.update", form.id), {
        onSuccess: () => close(),
        onError: () => show(),
    });
};
...
const destroy = async (role) => {
    ...
    if (response.isConfirmed) {
        ...
        return Inertia.delete(route("super_admin.role.destroy", role.id));
    }
};
        ...
        <Card class="bg-gray-50 dark:bg-gray-700 dark:text-gray-100">
            ...
            <template #body>
                <div class="flex flex-col space-y-2">
                    <Builder
                        v-if="render"
                        :url="route('super_admin.role.paginate')"
                        ref="table"
                    >
